<template>
    <section class="portfolio" id="portfolio">
      ...
      <div class="portfolio-content">
        <div class="row" v-for="(project, index) in projects" :key="index" ref="projectRows">
          <img :src="project.img" :alt="project.title" class="project-image" ref="projectImages">
          <div class="main-row">
            <div class="row-text">
...
const projects = [
  { img: require('@/img/crm.png'), title: 'CRM Project', category: 'Web App Development' },
  { img: require('@/img/task.webp'), title: 'Task Management', category: 'Web App Development' },
  { img: require('@/img/exp.png'), title: 'Expense Manager', category: 'Web App Development' }
];
  ...
  entries.forEach(entry => {
    if (entry.isIntersecting) {
      if (entry.target === ProjectsText.value) {
        gsap.fromTo(ProjectsText.value, 
          { opacity: 0, y: -50 }, 
      ...
      if (projectImages.value.includes(entry.target)) {
        gsap.fromTo(entry.target, 
          ...
          { opacity: 1, scale: 1, rotation: 0, duration: 1.5, stagger: 0.2, ease: 'power3.out' }
        );
      }
    }
  });
